...
function getBonusFp(ship) {
    ...
    if (itemNum(171)) {
        ...
        if (isIn(SHIP_YOMI, [US_SHIPS.BB, US_SHIPS.AC, US_SHIPS.CL])) {
            if (rfMax >= 10) {
                bonusFp += 1;
            }
        }
    ...
    if (num = itemNum(373)) {
        if (['ずいかく', 'たいほう', 'たいげい・りゅうほう'].includes(SHIP_YOMI) || ['飛鷹型', '千歳型', '最上型'].includes(SHIP_CLASS) || (SHIP_CLASS === '祥鳳型' && /改/.test(SHIP_NAME))) {
            bonusFp += num;
        } else if (SHIP_YOMI === 'しょうかく') {
            bonusFp += 2 * num;
        }
    ...
    if (num = itemNum(374)) {
        if (['祥鳳改', '龍鳳改', '飛鷹改'].includes(SHIP_NAME)) {
            ...
            bonusFp += num;
        } else if (['瑞鶴改二', '瑞鶴改二甲', '大鳳改'].includes(SHIP_NAME)) {
            bonusFp += 2 * num;
        } else if (['翔鶴改二', '翔鶴改二甲'].includes(SHIP_NAME)) {
            bonusFp += 3 * num;
        }
    }
    ...
    // 20.3cm(3号)連装砲=50
    if (num = itemNum(50)) {
        if (['古鷹型', '青葉型'].includes(SHIP_CLASS)) {
            bonusFp += num;
            bonusFp += isHaveX('surface') ? 1 : 0;
    ...
    if (num = itemNum(74)) {
        if (SHIP_YOMI === 'ゆきかぜ') {
            ...
            bonusFp += 2 * num;
        } else if (['ひえい', 'きりしま', 'ちょうかい', 'あかつき'].includes(SHIP_YOMI) || SHIP_NAME === '能代改二') {
            bonusFp += 4;
    ...
    if (num = itemNum(267)) {
        ...
        if (SHIP_CLASS === '夕雲型') {
            ...
            bonusFp += 2 * num;
            bonusFp += isHaveX('surface') && /改/.test(SHIP_NAME) ? 1 : 0;
    ...
    // 12.7cm連装砲D型改三=366
    if (num = itemNum(366)) {
        if (['秋雲改二', '沖波改二'].includes(SHIP_NAME)) {
